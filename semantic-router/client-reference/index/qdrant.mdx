...
- `include_metadata` (`bool`): Whether to include metadata in the returned Utterance objects.
...
```python
def get_routes_tuples() -> list[tuple]
```
...
`list[tuple]`: A list of (route_name, utterance, function_schemas, metadata) tuples.
